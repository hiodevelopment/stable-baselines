...
class ChessEnv(gym.Env):
    ...
    def __init__(self, player_color=1, opponent="random", log=True):
        ...
        self.moves_discrete = dict()
        k = 0
        for piece, id in pieces_to_ids.items():
            if id not in self.moves_discrete.keys():
                self.moves_discrete[id] = dict()
            for i in range(64):
                if i not in self.moves_discrete[id].keys():
                    self.moves_discrete[id][i] = dict()
                for j in range(64):
                    self.moves_discrete[id][i][j] = k
                    k += 1
    ...
    def step(self, action):
        ...
        if self.done:
            return self.state['board'], reward, self.done, {'state': self.state}
        ...
        #
            ...
            moves = ChessEnv.get_possible_moves(self.state, self.current_player)
            mask = [0] * 16**4
            for move in moves:
                mask[self.moves_discrete[move["piece_id"]][move["pos"][0] * 8 + move["pos"][1]][move["new_pos"][0] * 8 + move["new_pos"][1]]] = 1
            return self.state['board'], total_reward, self.done, {'state': self.state, "action_mask": mask}
